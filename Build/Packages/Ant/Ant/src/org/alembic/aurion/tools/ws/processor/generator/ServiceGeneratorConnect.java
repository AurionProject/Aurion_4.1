 ...
 */
public class ServiceGeneratorConnect extends GeneratorBase {
    public static void generate(Model model, WsimportOptions options, ErrorReceiver receiver) {
    	ServiceGeneratorConnect serviceGenerator = new ServiceGeneratorConnect(model, options, receiver);
        serviceGenerator.doGeneration();
    }

    private ServiceGeneratorConnect(Model model, WsimportOptions options, ErrorReceiver receiver) {
    ...
    public void visit(Service service) {
        ...
        JVar wsdlURL = staticBlock.decl(cm.ref(String.class), "wsdlURL", JExpr._null());
        ...
        tryBlock.body().assign(wsdlURL, cm.ref(ServicePropertyLoader.class).staticInvoke("getBaseWsdlPath"));
        tryBlock.body().directStatement(wsdlURL.name() + " = " + wsdlURL.name() + " + \"" + wsdlLocation + "\";");
        tryBlock.body().directStatement("logger.info(\"WSDL URL: '\" + " + wsdlURL.name() + " + \"'\");");
        tryBlock.body().assign(urlVar, JExpr._new(cm.ref(URL.class)).arg(baseUrl).arg(wsdlURL));

        JCatchBlock catchBlock = tryBlock._catch(cm.ref(MalformedURLException.class));
        catchBlock.param("e");

        catchBlock.body().directStatement("logger.warning(\"Failed to create URL for the wsdl Location: '\" + " + wsdlURL.name() + " + \"', retrying as a local file\");");
	...
	private String parseWsdlName(String inputValue) {
		String wsdlName = inputValue;
		try {
			if ((inputValue != null) && (inputValue.endsWith(".wsdl"))) {
				int lastIndex = inputValue.lastIndexOf("\\");
				if (lastIndex == -1) {
					lastIndex = inputValue.lastIndexOf("/");
				}
				wsdlName = "/wsdl/" + inputValue.substring(lastIndex + 1);
			}
		} catch (Throwable t) {
			wsdlName = inputValue;
		}
