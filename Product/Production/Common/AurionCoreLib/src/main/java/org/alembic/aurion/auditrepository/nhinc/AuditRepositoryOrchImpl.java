...
import org.alembic.aurion.hibernate.AuditRepositoryDAO;
import org.alembic.aurion.hibernate.AuditRepositoryRecord;
...
public class AuditRepositoryOrchImpl {
     ...
     * @param mess
     * @return org.alembic.aurion.common.nhinccommon.AcknowledgementType
     */
    public AcknowledgementType logAudit(LogEventSecureRequestType mess, AssertionType assertion) {
        ...
        if (participantObjectIdentificationList != null && participantObjectIdentificationList.size() > 0) {
            ...
            if (participantObjectIdentificationType != null) {
                ...
                if (null != eventPatientID) {
                    eventPatientID = PatientIdFormatUtil.parsePatientId(eventPatientID);
        ...
        if (null != eventIdentification) {
            CodedValueType sEventId = eventIdentification.getEventID();
            if (null != sEventId && null != sEventId.getCode() && !sEventId.getCode().equals("")) {
                try{
                    auditRec.setEventId(Integer.parseInt(sEventId.getCode()));
                }catch(NumberFormatException exp){
                    log.error("Error occured: "+exp.getMessage());
                }
            }
    ...
    public FindCommunitiesAndAuditEventsResponseType findAudit(FindAuditEventsType query, AssertionType assertion) {
        ...
        patientId = PatientIdFormatUtil.parsePatientId(patientId);
