 ...
 */
package org.alembic.aurion.webserviceproxy;

import com.sun.xml.ws.api.message.Header;
import com.sun.xml.ws.developer.WSBindingProvider;
import org.alembic.aurion.async.AsyncHeaderCreator;
import org.alembic.aurion.tools.ws.processor.generator.ServicePropertyLoader;
...
public class WebServiceProxyHelper {
    ...
    private List<Header> createdHeaders = null;
    ...
    protected List getWSAddressingHeaders(String url, String wsAddressingAction, AssertionType assertion) {
        ...
        List<Header> lCreatedHeaders = hdrCreator.createOutboundHeaders(url, wsAddressingAction, messageId, allRelatesTo);
        return lCreatedHeaders;
    }
     ...
     * @param lCreatedHeaders The listing of WS-Addressing headers.
     */
    protected void setOutboundHeaders(BindingProvider port, List<Header> lCreatedHeaders) {
        ((WSBindingProvider) port).setOutboundHeaders(lCreatedHeaders);
    }
    ...
    private void initializePort(boolean isSecure, BindingProvider port, String url, String serviceAction,
        ...
        if (wsAddressingAction != null) {
            createdHeaders = getWSAddressingHeaders(url, wsAddressingAction, assertion);
    ...
    public Object invokePort(Object portObject, Class portClass, String methodName, Object operationInput)
        ...
        InvocationTargetException eCatchExp = null;
        String msgId = getMessageIdFromHeadersList();
        ...
        if (oMethod != null) {
            if ((iRetryCount > 0) &&
                ...
                int i = 1;
                AuditPerformance oAuditPerformance = PerformanceMonitorUtil.buildAuditPerfromance(i, portClass.getSimpleName(), msgId, "Request");
                ...
                while (i <= iRetryCount) {
                    try {
                        ...
                        if (isRetryException(e)) {
                            ...
                            //---------------------------------------------------------------------
                            String sErrorMessage = "(1) An unexpected exception occurred of type: " +
                        ...
                        //---------------------------------------------------------------------
                        String sErrorMessage = "(2) An unexpected exception occurred of type: " +
                                e.getClass().getCanonicalName() + ". Exception: " +
                                e.getMessage();
                        log.error(sErrorMessage, e);
                        throw e;
                ...
                oAuditPerformance = PerformanceMonitorUtil.buildAuditPerfromance(i, portClass.getSimpleName(), msgId, "Response");
                PerformanceMonitorUtil.getPerformanceProxy().logPerformance(oAuditPerformance);
            } // if ((iRetryCount > 0) && (iRetryDelay > 0))
            else {
                try {
                    log.debug("Invoking " + portClass.getCanonicalName() + "." + oMethod.getName() + ": Retry is not being used");
                    AuditPerformance oAuditPerformance = PerformanceMonitorUtil.buildAuditPerfromance(0, portClass.getSimpleName(), msgId, "Request");
                    PerformanceMonitorUtil.getPerformanceProxy().logPerformance(oAuditPerformance);
                    oResponse = invokeTheMethod(oMethod, portObject, operationInput);
                    oAuditPerformance = PerformanceMonitorUtil.buildAuditPerfromance(0, portClass.getSimpleName(), msgId, "Response");
                    PerformanceMonitorUtil.getPerformanceProxy().logPerformance(oAuditPerformance);
                } catch (IllegalArgumentException e) {
                    String sErrorMessage = "The method was called with incorrect arguments. " +
                            "This assumes that the method should have exactly one " +
                    ...
                    // log them if we see them.
                    //---------------------------------------------------------------------
                    String sErrorMessage = "(3) An unexpected exception occurred of type: " +
                            e.getClass().getCanonicalName() + ". Exception: " +
                            e.getMessage();
                    log.error(sErrorMessage, e);
                    throw e;
    ...
    /*
     *
     * Simple recursive function to traverse nested exceptions to find if the faulting exceptions is one which should cause us to
     * Retry the request. Added to address user story: asrt1020
     *
     * @param exp The exception to check
     * @return true if the exception or one of its nested causes should result in a retry attempt
     */
    private boolean isRetryException(Throwable exp) {
//        if (exp instanceof javax.xml.ws.WebServiceException)
//        {
//            //Case carried over from 3.1 code base
//            return true;
//        }
//        else
        log.debug("Recursively checking for retry exception.  Exception class: " + exp.getClass().getCanonicalName());
        if (exp instanceof java.net.SocketTimeoutException) {
            log.debug("Found instance of java.net.SocketTimeoutException.");
            return true;
        } else if (exp.getCause() != null) {
            return isRetryException(exp.getCause());
        }
     ...
     * @return String
     */
    private String getMessageIdFromHeadersList() {
        String msgId = "";
        if (createdHeaders != null && createdHeaders.size() > 0) {
            for (Header header : createdHeaders) {
                if (header.getLocalPart().equalsIgnoreCase(NhincConstants.WS_SOAP_HEADER_MESSAGE_ID)) {
                    msgId = header.getStringContent();
                }
            }
        }
        msgId = (msgId != null && !msgId.equals("")) ? msgId : "0";
        return msgId;
    }
}
