...
//        }
        ...
        if (assertion != null) {
            appendAssertionIssuer(policyRequest, assertion);
            ...
            {
                log.debug("Saml Authz Decision Statement Not Null...");
    ...
    private void appendAssertionIssuer(RequestType policyRequest, AssertionType assertion) {
        log.debug("begin appending AssertionIssuer");
        SubjectType parent = getSubject(policyRequest);
        String attributeId = XacmlAttributeId.AssertionIssuer;
        String dataType = Constants.DataTypeString;
        String attributeValue = extractAssertionIssuer(assertion);
        AttributeHelper attrHelper = new AttributeHelper();
        attrHelper.appendAttributeToParent(parent, attributeId, dataType, attributeValue, appendAttributesIfNull);
        log.debug("end appending AssertionIssuer");
    }

    private String extractAssertionIssuer(AssertionType assertion) {
        String value = null;
        if (assertion.getSamlIssuer() != null) {
            log.debug("Extracting AssertionIssuer");
            value = assertion.getSamlIssuer().getIssuer();
            log.debug("Extracted AssertionIssuer value=" + value);
        } else {
            log.warn("Unable to find Assertion Issuer in SAML, will not be included in message to policy engine");
        }
