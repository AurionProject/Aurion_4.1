...
import org.alembic.aurion.patientcorrelation.nhinc.dao.CorrelatedIdentifiersDao;
import org.alembic.aurion.patientcorrelation.nhinc.model.CorrelatedIdentifiers;
import org.alembic.aurion.patientcorrelation.nhinc.model.QualifiedPatientIdentifier;
import org.alembic.aurion.patientcorrelation.nhinc.parsers.PRPAIN201309UV.PRPAIN201309UVParser;
import org.alembic.aurion.patientcorrelation.nhinc.ack.AckBuilder;
import org.alembic.aurion.patientcorrelation.nhinc.config.ConfigurationManager;
import org.alembic.aurion.patientcorrelation.nhinc.config.Expiration;
import org.alembic.aurion.patientcorrelation.nhinc.config.ExpirationConfiguration;
import org.alembic.aurion.patientcorrelation.nhinc.parsers.PRPAIN201301UV.PRPAIN201301UVParser;
import org.alembic.aurion.patientcorrelation.nhinc.parsers.PRPAIN201309UV.PixRetrieveResponseBuilder;
...
public class PatientCorrelationOrchImpl {
    ...
    public RetrievePatientCorrelationsResponseType retrievePatientCorrelations(PRPAIN201309UV02 retrievePatientCorrelationsRequest, AssertionType assertion) {
        ...
        List<String> allowableHomeCommunityIds = extractDataSourceList(retrievePatientCorrelationsRequest);
        List<String> allowableAssigningAuthorities = lookupAssigningAuthoritiesFromHomeCommunities(allowableHomeCommunityIds);
        ...
        List<QualifiedPatientIdentifier> qualifiedPatientIdentifiers = dao.retrievePatientCorrelation(inputQualifiedPatientIdentifier , allowableAssigningAuthorities);
    ...
    private List<String> lookupAssigningAuthoritiesFromHomeCommunities(List<String> homeCommunityIds) {
        ...
        if(homeCommunityIds != null) {
            ...
            assigningAuthorities = new ArrayList<String>();
            for(String homeCommunityId : homeCommunityIds) {
                List<String> aaListForHomeCommunityId = mappingDao.getAssigningAuthoritiesByHomeCommunity(homeCommunityId);
                if(aaListForHomeCommunityId != null) {
                    assigningAuthorities.addAll(aaListForHomeCommunityId);
                }
            }
        }
