...
import org.alembic.aurion.patientdiscovery.response.TrustMode;
import org.alembic.aurion.patientdiscovery.response.VerifyMode;
...
public class NhinPatientDiscoveryDeferredRespOrchImpl {
    ...
    {
        log.debug("NhinPatientDiscoveryDeferredRespOrchImpl.respondingGatewayPRPAIN201306UV02Orch() -- Begin");
        ...
        if (isNhinAudit) {
            ack = auditLogger.auditNhin201306(body, assertion, NhincConstants.AUDIT_LOG_INBOUND_DIRECTION);
        }
        ...
        log.debug("NhinPatientDiscoveryDeferredRespOrchImpl.respondingGatewayPRPAIN201306UV02Orch() -- End");
        return resp;
    }
    ...
    {
        log.debug("NhinPatientDiscoveryDeferredRespOrchImpl.sendToAdapter() -- Begin");
        boolean isAdapterAudit = getPropertyBoolean(NhincConstants.GATEWAY_PROPERTY_FILE, NhincConstants.ADAPTER_AUDIT_PROPERTY);
        PatientDiscoveryAuditLogger auditLogger = new PatientDiscoveryAuditLogger();
        AcknowledgementType ack = null;
        if (isAdapterAudit) {
            ack = auditLogger.auditAdapter201306(body, assertion, NhincConstants.AUDIT_LOG_OUTBOUND_DIRECTION);
        }
        ...
        log.debug("NhinPatientDiscoveryDeferredRespOrchImpl.sendToAdapter() -- End");
        return resp;
    }
    ...
    protected void processRespTrustMode(PRPAIN201306UV02 body, AssertionType assertion)
    {
        ...
        List<AsyncMsgRecord> asyncMsgRecs = asyncDbDao.queryByMessageId(assertion.getRelatesToList().get(0));
