...
import org.alembic.aurion.docquery.adapter.proxy.AdapterDocQueryProxy;
import org.alembic.aurion.docquery.adapter.proxy.AdapterDocQueryProxyObjectFactory;
...
public class NhinDocQueryOrchImpl {
    ...
    public AdhocQueryResponse respondingGatewayCrossGatewayQuery(AdhocQueryRequest body, AssertionType assertion) {
        ...
        if (isNhinAudit) {
            ack = auditAdhocQueryRequest(crossGatewayQueryRequest, NhincConstants.AUDIT_LOG_INBOUND_DIRECTION, NhincConstants.AUDIT_LOG_NHIN_INTERFACE);
        }
    ...
    private AdhocQueryResponse forwardToAgency(RespondingGatewayCrossGatewayQueryRequestType adhocQueryRequestMsg)
    {
        ...
        if (isAdapterAudit) {
            ack = auditAdhocQueryRequest(adhocQueryRequestMsg, NhincConstants.AUDIT_LOG_OUTBOUND_DIRECTION, NhincConstants.AUDIT_LOG_ADAPTER_INTERFACE);
        }

        org.alembic.aurion.common.nhinccommonadapter.RespondingGatewayCrossGatewayQueryRequestType crossGatewayQueryEventsRequest = new org.alembic.aurion.common.nhinccommonadapter.RespondingGatewayCrossGatewayQueryRequestType();
        ...
        // Audit the Audit Log Query Response Message received on the Nhin Interface
        if (isAdapterAudit) {
            ack = auditAdhocQueryResponse(resp, NhincConstants.AUDIT_LOG_INBOUND_DIRECTION, NhincConstants.AUDIT_LOG_ADAPTER_INTERFACE, adhocQueryRequestMsg.getAssertion());
        }
