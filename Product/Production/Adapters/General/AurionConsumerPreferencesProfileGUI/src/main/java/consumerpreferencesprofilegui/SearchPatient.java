...
import org.alembic.aurion.adapter.cppgui.CPPConstants;
import org.alembic.aurion.adapter.cppgui.servicefacade.AdapterPIPFacade;
import org.alembic.aurion.adapter.cppgui.ConsumerPreferencesSearchCriteria;
...
import org.alembic.aurion.adapter.cppgui.servicefacade.PatientSearchFacade;
import org.alembic.aurion.adapter.cppgui.valueobject.PatientVO;
import org.alembic.aurion.adapter.cppgui.UserSession;
...
public class SearchPatient extends AbstractPageBean {
    ...
    public String savePreferences() {
        ...
        PatientPreferencesVO oldPtPreferences = patientVO.getPatientPreferences();

        PatientPreferencesVO patientPreferencesVO = new PatientPreferencesVO();

        // Need to keep the old policy OID so that the record can be updated.
        //-------------------------------------------------------------------
        if (oldPtPreferences != null)
        {
            patientPreferencesVO.setConsentPolicyOID(oldPtPreferences.getConsentPolicyOID());
            log.debug("SearchPatient.savePreferences: patientPrefernecesVO.ConsentPolicyOID = '" +
                      patientPreferencesVO.getConsentPolicyOID() + "'");
        }
        else
        {
            log.debug("SearchPatient.savePreferences: patientPrefernecesVO.ConsentPolicyOID = DID NOT EXIST");
        }
